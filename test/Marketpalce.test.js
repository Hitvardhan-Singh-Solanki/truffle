...
contract("Marketplace", ([master, seller, buyer]) => {
  let marketplace;
  ...
  describe("deployment", async () => {
    ...
    it("has a name", async () => {
      ...
      assert.equal(name, "EY marketplace");
    });
  });
  ...
  describe("products", async () => {
    ...
    it("should create a product", async () => {
      ...
      await marketplace.createProduct("laptop", 0, { from: seller }).should.be
        .rejected;
    });
    ...
    it("sells products", async () => {
      ...
      const event = result.logs[0].args;
      assert.equal(
        event.productCount.toNumber(),
        productCount.toNumber(),
        "id is correct"
      );
      assert.equal(event.productName, "laptop", "name is correct");
      assert.equal(
        event.productPrice,
        "1000000000000000000",
        "price is correct"
      ...
      // FAILURE: master tries to buy the product, i.e., product can't be purchased twice
      await marketplace.purchaseProduct(productCount, {
        from: master,
        value: web3.utils.toWei("1", "Ether")
      }).should.be.rejected;
